...
function init(){
  ...
  document.addEventListener('wheel', mouseWheel,false); /* 마우스 휠 활성화시 호출*/
...
function mouseUp(e){ /* 마우스를 누른 상태에서 때었을 때 멈춰있던 화면의 시점이 돌아감*/
  info.isUserInteracting = false; /* 마우스 클릭 유무의 판별 변수값 마우스를 누르고 있지 않다.*/
}
function mouseMove(e){ /* 마우스를 움직일때인데 if조건에 의해 마우스를 누른 상태에서만 호출하게 됨.*/
  if(!info.isUserInteracting) return; /* 마우스를 누른 상태가 아니라면 리턴*/
...
function mouseDown(e){
  ...
  info.isUserInteracting = true; /* 마우스 클릭 유무의 판별 변수값 마우스를 누르고 있다.*/
  
  info.onPointerDownPointerX = e.clientX;  /* 마우스를 눌렀을 때 해당 위치의 x점과 y점이 포인터에 저장됨*/
  info.onPointerDownPointerY = e.clientY; /* 마우스 무브를 위함*/
  
  info.onPointerDownLon = info.lon; /* 경도 포인터에 담음*/
  info.onPointerDownLat = info.lat; /* 위도 포인터에 담음 마우스 무브점을 위함*/
}
...
function render(){ /* 말 그대로 루프가 도는 렌더임 랜딩되는 요소들 모아놓음 정리*/
  ...
  if(info.isUserInteracting === false) info.lon += 0.1; /* 만약 마우스를 누르고 있지 않을 시 경도를 조정함*/
